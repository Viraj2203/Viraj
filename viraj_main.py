...
thresh = cv2.adaptiveThreshold(blur, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 45, 23)
...
for i in range(0, len(contours)):
    cv2.drawContours(imageContours, contours, i, (255, 255, 255)) # draw contours based on actual found contours of thresh image
    possibleChar = Functions.ifChar(contours[i]) # retrieve a possible char by the result ifChar class give us
   
    if Functions.checkIfChar(possibleChar) is True: # by computing some values (area, width, height, aspect ratio) possibleChars list is being populated
...
for possibleC in possibleChars:
    ...
    def matchingChars(possibleC, possibleChars):
        ...
        for possibleMatchingChar in possibleChars:
            ...
            if distanceBetweenChars < (possibleC.diagonalSize * 5.4) and \
...
for listOfMatchingChars in listOfListsOfMatchingChars:
    ...
    for i in range(0, len(plates_list)):
        ...
        '''Final plate detection'''
        cv2.imshow("plate", plates_list[i].Plate)
        cv2.imwrite(temp_folder + '13 - plate.png', plates_list[i].Plate)


#Charater detction
img = Image.open('13 - plate.png')
char=pytesseract.image_to_string(Image.open('13 - plate.png'))
l=[]
for i in range (len(char)):
    if(48<=ord(char[i])<=57 or 65<= ord(char[i])<=90):
        a=char[i]
        l.append(a)
print("".join(l)) 
